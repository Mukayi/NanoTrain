    ...
    def train(self) -> None:
        ...
        while True:
            ...
            if self.iter_num % self.config.train.log_interval == 0:
                lossf = loss.item() * self.config.train.gradient_accumulation_steps
                if local_iter_num >= 5:
                    mfu = self.raw_model.estimate_mfu(
                        self.config.data.batch_size * self.config.train.gradient_accumulation_steps,
                        dt,
                    )
                    self.running_mfu = (
...
def run_training(config: NanoTrainConfig, project_root: str | Path | None = None) -> None:
    Trainer(config, project_root=project_root).train()
